...
public class ProfessorController {
	...
	void addCategory(ActionEvent event) {
		...
		try {
			...
			String jndiName3="PFE-ear/PFE-ejb/CategoryFacade!iservice.CategoryFacadeRemote";
			CategoryFacadeRemote proxy3=(CategoryFacadeRemote) context.lookup(jndiName3);
	...
	void choisir( ActionEvent evt) throws NamingException{
		Context context=new InitialContext();
		String jndiName2="PFE-ear/PFE-ejb/EmployeeFacade!iservice.EmployeeFacadeRemote";
		EmployeeFacadeRemote proxy2=(EmployeeFacadeRemote ) context.lookup(jndiName2);
		...
		if(nom_c!=null){
			...
			proxy2.choisir_categorie(c.get(0).getCategory_id(), employee.getEmployee_id());
			Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
			alert.setTitle(" information");
			alert.setHeaderText(null);
			alert.setContentText("Category choosen  !");
			alert.show();
		}
		else{
			Alert alert = new Alert(Alert.AlertType.WARNING);
	...
	void showSupAction (ActionEvent event) throws NamingException {
		...
		Context context=new InitialContext();
		String jndiName1="PFE-ear/PFE-ejb/PFE_FormFacade!iservice.PFE_FormFacadeRemote";
		PFE_FormFacadeRemote proxy1=(PFE_FormFacadeRemote) context.lookup(jndiName1);
		List<PFE_Form> l=proxy1.afficher_supervision(employee.getEmployee_id());
		if(l.size()!=0)
		{
			ObservableList<PFE_Form> l1=FXCollections.observableArrayList();
			l1.addAll(l);
			for(int i=0;i<l.size();i++){
				resp_mail_s.setCellValueFactory(new PropertyValueFactory<PFE_Form,String>("responsibleEmail"));
				resp_name_s.setCellValueFactory(new PropertyValueFactory<PFE_Form,String>("responsibleFirstName"));
				title_pfe_s.setCellValueFactory(new PropertyValueFactory<PFE_Form,String>("title"));
				prob_s.setCellValueFactory(new PropertyValueFactory<PFE_Form,String>("problematic"));
	...
	void showRepAction(ActionEvent event) throws NamingException {
		...
		String jndiName1="PFE-ear/PFE-ejb/PFE_FormFacade!iservice.PFE_FormFacadeRemote";
		PFE_FormFacadeRemote proxy1=(PFE_FormFacadeRemote) context.lookup(jndiName1);
		List<PFE_Form> l=proxy1.afficher_reporter(employee.getEmployee_id());
		if(l.size()!=0)
		{
			ObservableList<PFE_Form> l1=FXCollections.observableArrayList();
			l1.addAll(l);
